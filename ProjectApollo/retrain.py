...
MIN_NUM_IMAGES_REQUIRED_FOR_TRAINING = 3
...
TRAINING_IMAGES_DIR = '/home/swati/Desktop/Development_3/new_dataset/train'
...
def main():
    ...
    if class_count == 1:
        tf.logging.error(
            'Only one valid folder of images found at ' + TRAINING_IMAGES_DIR + ' - multiple classes are needed for classification.')
        return -1
    # end if
    ...
    with tf.Session(graph=graph) as sess:
        ...
        if doDistortImages:
            ...
            (distorted_jpeg_data_tensor, distorted_image_tensor) = add_input_distortions(FLIP_LEFT_RIGHT, RANDOM_CROP,
                                                                                         RANDOM_SCALE,
                                                                                         RANDOM_BRIGHTNESS,
            ...
            # cached them on disk.
            cache_bottlenecks(sess, image_lists, TRAINING_IMAGES_DIR, BOTTLENECK_DIR, jpeg_data_tensor,
        ...
        for i in range(HOW_MANY_TRAINING_STEPS):
            ...
            if (i % EVAL_STEP_INTERVAL) == 0 or is_last_step:
                ...
                validation_bottlenecks, validation_ground_truth, _ = (
                    get_random_cached_bottlenecks(sess, image_lists, VALIDATION_BATCH_SIZE, 'validation',
        ...
        test_bottlenecks, test_ground_truth, test_filenames = (
            get_random_cached_bottlenecks(sess, image_lists, TEST_BATCH_SIZE, 'testing', BOTTLENECK_DIR,
                                          TRAINING_IMAGES_DIR, jpeg_data_tensor, decoded_image_tensor,
                                          resized_image_tensor,
...
def checkIfNecessaryPathsAndFilesExist():
    ...
    for trainingSubDir in trainingSubDirs:
        if trainingSubDir.numImages < MIN_NUM_IMAGES_REQUIRED_FOR_TRAINING:
            print("ERROR: there are less than the required " + str(
                MIN_NUM_IMAGES_REQUIRED_FOR_TRAINING) + " images in " + trainingSubDir.loc)
    ...
    for trainingSubDir in trainingSubDirs:
        if trainingSubDir.numImages < MIN_NUM_IMAGES_SUGGESTED_FOR_TRAINING:
            print("WARNING: there are less than the suggested " + str(
                MIN_NUM_IMAGES_SUGGESTED_FOR_TRAINING) + " images in " + trainingSubDir.loc)
    ...
    if numImagesInTestDir < MIN_NUM_IMAGES_REQUIRED_FOR_TESTING:
        print("ERROR: there are not at least " + str(
            MIN_NUM_IMAGES_REQUIRED_FOR_TESTING) + " images in " + TEST_IMAGES_DIR)
        print("Did you break out some test images?")
        return False
    # end if
...
def create_model_info(architecture):
    ...
    if architecture == 'inception_v3':
        ...
        if (
                version_string != '1.0' and version_string != '0.75' and version_string != '0.50' and version_string != '0.25'):
            tf.logging.error(
    ...
    return {'data_url': data_url, 'bottleneck_tensor_name': bottleneck_tensor_name,
            ...
            'model_file_name': model_file_name, 'input_mean': input_mean, 'input_std': input_std,
...
def downloadModelIfNotAlreadyPresent(data_url):
    ...
    if not os.path.exists(filepath):
        ...
        tf.logging.info(
            'Successfully downloaded ' + str(filename) + ', statinfo.st_size = ' + str(statinfo.st_size) + ' bytes')
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    for sub_dir in sub_dirs:
        ...
        if len(file_list) < 20:
            ...
            tf.logging.warning(
                'WARNING: Folder {} has more than {} images. Some images will never be selected.'.format(dir_name,
